...
public class Size {
	...
	public static void changeSize(File photo) {
		System.out.println("Start change the size of photo: " + photo.getName());
		try {
			...
			BufferedImage image2 = new BufferedImage(sizeWidth, sizeHeight, BufferedImage.TYPE_3BYTE_BGR);

			for (int i = 0; i < orgWidth; i += orgWidth / sizeWidth) {
				for (int j = 0; j < orgHeight; j += orgHeight / sizeHeight) {
					image2.setRGB(i * sizeWidth / orgWidth, j * sizeHeight / orgHeight, image.getRGB(i, j));
				}
			}

			ImageIO.write(image2, "jpg", file2);
			System.out.println("Change the size of picture:" + photo.getName() + "Finish.");
			photoCount++;
		} catch (IOException e) {
			e.printStackTrace();
		}
